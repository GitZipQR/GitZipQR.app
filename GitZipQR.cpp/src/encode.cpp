...
// При директории — запаковываем её содержимое в zip (без верхней папки).
static std::string materialize_data(const std::string& input,std::string& nameBase,std::string& metaExt){
  if(is_dir(input)){
    ...
    std::string cmd = "(cd '" + input + "' && zip -r -q '" + out + "' .)";
                                    ...
                                    const std::string& metaExt)
{
  ...
  const int worstChunk = 999999, worstTotal = 999999, worstChunkSize = 999999;
  ...
  auto fits = [&](int dataBytes)->bool{
    ...
    meta["cipherHash"] = std::string(64,'0');
...
int main(int argc,char** argv){
  ...
  try{
    ...
    std::string pass = std::getenv("GZQR_PASS") ? std::getenv("GZQR_PASS") : gzqr_config::getDefaultPassword();
    ...
    { FILE* tf=fopen(dataPath.c_str(),"rb"); if(!tf){ std::fprintf(stderr,"Error: cannot open input for encrypt: %s\n", dataPath.c_str()); return 1; } fclose(tf); }
